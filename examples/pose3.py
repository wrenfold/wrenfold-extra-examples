...
class Pose3:
    ...
    def retract(self, vec: Vector6, epsilon: T.Union[int, float] = 1.0e-16) -> "Pose3":
        """
    ...
    def retract_first_order(self, vec: Vector6) -> "Pose3":
        """
    ...
    def local_coordinates(self, other: "Pose3", epsilon: T.Union[int, float] = 1.0e-16) -> Vector6:
        """
        ...
        u_vec = (
            geometry.inverse_left_jacobian_of_so3(w=w_vec, epsilon=epsilon) *
            delta_pose.translation)
        # Turn into one 6x1 vector ordered (rotation, translation).
        return sym.vstack([w_vec, u_vec])

    def local_coordinates_first_order(self, other: "Pose3") -> Vector6:
        """
    ...
    def compose(self, other: "Pose3") -> "Pose3":
        ...
        """
        return Pose3(
            rotation=Rot3.from_quaternion(self.rotation.to_quaternion() *
    ...
    def right_retract_derivative(self) -> sym.MatrixExpr:
        ...
        substitutions = list(zip(variables, self.to_vector()))
...
@functools.cache
def se3_right_retract_derivative() -> T.Tuple[T.List[sym.Expr], sym.MatrixExpr]:
    """
...
@functools.cache
def se3_right_local_coordinates_derivative() -> (T.Tuple[T.List[sym.Expr], sym.MatrixExpr]):
    """
