// This is an example of interfacing wrenfold generated functions with Sophus types.
// We don't have anything to optimize here, so instead we will compare our generated function
// to a GTSAM implementation.
#include <fmt/ostream.h>
#include <gtest/gtest.h>
#include <gtsam/base/numericalDerivative.h>
#include <gtsam/geometry/Pose3.h>
#include <Eigen/Geometry>
#include <sophus/se3.hpp>

#include "generated/pose3_interpolate.h"

TEST(SophusPose3InterpolationTest, TestInterpolation) {
  const Sophus::SE3d world_T_a{
      Eigen::Quaterniond(0.410182, 0.280694, 0.857538, 0.132633).normalized(),
      Eigen::Vector3d{-2.0, 3.4, 0.7}};
  const Sophus::SE3d world_T_b{
      Eigen::Quaterniond(0.772165, -0.596797, -0.191188, 0.105078).normalized(),
      Eigen::Vector3d{1.2, -4.32, 0.1}};

  for (const double alpha : {0.0, 0.001, 0.5, 0.999, 1.0}) {
    Eigen::Matrix<double, 6, 6> D_a_analytical, D_b_analytical;

    // Invoke the generated version:
    const Sophus::SE3d world_T_interpolated =
        gen::pose3_interpolate(world_T_a, world_T_b, alpha, D_a_analytical, D_b_analytical);

    // Compute the interpolated pose using gtsam as well, and compare
    Eigen::Matrix<double, 6, 6> D_a_gtsam, D_b_gtsam;
    const gtsam::Pose3 world_T_interoplated_gtsam = gtsam::interpolate(
        gtsam::Pose3{gtsam::Rot3(world_T_a.so3().unit_quaternion()), world_T_a.translation()},
        gtsam::Pose3{gtsam::Rot3(world_T_b.so3().unit_quaternion()), world_T_b.translation()},
        alpha, &D_a_gtsam, &D_b_gtsam);

    ASSERT_LE(
        (world_T_interpolated.matrix() - world_T_interoplated_gtsam.matrix()).cwiseAbs().maxCoeff(),
        1.0e-12)
        << fmt::format("generated:\n{}\ngtsam:\n{}\n", fmt::streamed(world_T_interpolated.matrix()),
                       fmt::streamed(world_T_interoplated_gtsam.matrix()));

    ASSERT_LE((D_a_analytical - D_a_gtsam).cwiseAbs().maxCoeff(), 1.0e-9) << fmt::format(
        "generated:\n{}\ngtsam:\n{}\n", fmt::streamed(D_a_analytical), fmt::streamed(D_a_gtsam));

    ASSERT_LE((D_b_analytical - D_b_gtsam).cwiseAbs().maxCoeff(), 1.0e-9) << fmt::format(
        "generated:\n{}\ngtsam:\n{}\n", fmt::streamed(D_b_analytical), fmt::streamed(D_b_gtsam));
  }
}

TEST(SophusPose3InterpolationTest, TestInterpolationFirstOrder) {
  const Sophus::SE3d world_T_a{
      Eigen::Quaterniond(0.410182, 0.280694, 0.857538, 0.132633).normalized(),
      Eigen::Vector3d{-2.0, 3.4, 0.7}};
  const Sophus::SE3d world_T_b{
      Eigen::Quaterniond(0.772165, -0.596797, -0.191188, 0.105078).normalized(),
      Eigen::Vector3d{1.2, -4.32, 0.1}};

  for (const double alpha : {0.0, 0.001, 0.5, 0.999, 1.0}) {
    Eigen::Matrix<double, 6, 6> D_a_analytical, D_b_analytical;

    // Invoke the generated version:
    const Sophus::SE3d world_T_interpolated = gen::pose3_interpolate_first_order(
        world_T_a, world_T_b, alpha, D_a_analytical, D_b_analytical);

    // Compute the interpolated pose using gtsam as well, and compare
    Eigen::Matrix<double, 6, 6> D_a_gtsam, D_b_gtsam;
    const gtsam::Pose3 world_T_interoplated_gtsam = gtsam::interpolate(
        gtsam::Pose3{gtsam::Rot3(world_T_a.so3().unit_quaternion()), world_T_a.translation()},
        gtsam::Pose3{gtsam::Rot3(world_T_b.so3().unit_quaternion()), world_T_b.translation()},
        alpha, &D_a_gtsam, &D_b_gtsam);

    ASSERT_LE(
        (world_T_interpolated.matrix() - world_T_interoplated_gtsam.matrix()).cwiseAbs().maxCoeff(),
        1.0e-12)
        << fmt::format("generated:\n{}\ngtsam:\n{}\n", fmt::streamed(world_T_interpolated.matrix()),
                       fmt::streamed(world_T_interoplated_gtsam.matrix()));

    ASSERT_LE((D_a_analytical - D_a_gtsam).cwiseAbs().maxCoeff(), 1.0e-9) << fmt::format(
        "generated:\n{}\ngtsam:\n{}\n", fmt::streamed(D_a_analytical), fmt::streamed(D_a_gtsam));

    ASSERT_LE((D_b_analytical - D_b_gtsam).cwiseAbs().maxCoeff(), 1.0e-9) << fmt::format(
        "generated:\n{}\ngtsam:\n{}\n", fmt::streamed(D_b_analytical), fmt::streamed(D_b_gtsam));
  }
}
